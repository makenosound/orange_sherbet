...
module OrangeSherbet
  ...
  # flow are real statements. Three value-dependent helpers — __esc (HTML
  # escape), __s (nil-safe to_s), __truthy (Ruby truthiness) — live in a shared
  # runtime module (RUNTIME); each template imports only the ones it uses.
  class Compiler
    include Js

    # The shared runtime — three tiny value helpers the compiled templates use.
    # Emitted once per output directory (see CLI / OrangeSherbet.runtime); each
    # template imports only the ones it uses, rather than inlining them.
    RUNTIME_FILENAME = "sherbet-runtime.js"
    ...
    def compile(name, source)
      ...
      unless @helpers.empty?
        printer.line("import { #{@helpers.uniq.sort.join(", ")} } from \"./#{RUNTIME_FILENAME}\";")
      end
      printer.line("")
    ...
    def emit_stmt(node)
      ...
      when :output
        ...
        wrapped = j_call(h(safe ? "__s" : "__esc"), [emit_expr(expr)])
    ...
    # Reference a runtime helper, recording it so the template imports only the
    ...
    def truthy(prism_node)
      j_call(h("__truthy"), [emit_expr(prism_node)])
    end
    ...
    def emit_expr(node)
      ...
      when Prism::OrNode
        ...
        arrow = j_arrow(["__l"], j_ternary(j_call(h("__truthy"), [j_ident("__l")]),
      ...
      when Prism::InterpolatedStringNode
        parts = node.parts.map do |part|
          part.is_a?(Prism::StringNode) ? part.unescaped : j_call(h("__s"), [emit_expr(part.statements.body.first)])
        end
        j_tpl(parts)
    ...
    def emit_call(node)
      ...
      return j_unary("!", j_call(h("__truthy"), [recv])) if node.name == :!
    ...
    def rewrite_method(name, r, a)
      ...
      when :join then j_call(j_member(j_call(j_member(r, "map"), [h("__s")]), "join"), [a.first || j_str("")])
